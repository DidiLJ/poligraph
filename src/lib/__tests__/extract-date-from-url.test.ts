...
describe("/article/YYYY/MM/DD/ (Le Monde, HuffPost)", () => {
  it("extracts date from a realistic Le Monde article URL", () => {
    ...
    expect(extractDateFromUrl(url)!.toISOString().slice(0, 10)).toBe("2024-03-15");
  });
});
...
describe("/YYYY/MM/DD/ (HuffPost-style)", () => {
  it("extracts date from a realistic HuffPost France URL", () => {
    ...
    expect(extractDateFromUrl(url)!.toISOString().slice(0, 10)).toBe("2024-06-20");
  });
  ...
  it("accepts boundary year 2000", () => {
    expect(
      extractDateFromUrl("https://www.huffingtonpost.fr/2000/01/01/debut/")!
        .toISOString()
        .slice(0, 10)
    ).toBe("2000-01-01");
  });
  ...
  it("accepts boundary year 2030", () => {
    expect(
      extractDateFromUrl("https://www.huffingtonpost.fr/2030/12/31/fin/")!
        .toISOString()
        .slice(0, 10)
    ).toBe("2030-12-31");
  });
...
describe("-YYYYMMDD_ / -YYYYMMDD- (Libération, France24)", () => {
  it("extracts date from Libération-style -YYYYMMDD_", () => {
    ...
    expect(extractDateFromUrl(url)!.toISOString().slice(0, 10)).toBe("2024-03-18");
  });

  it("extracts date from France24-style -YYYYMMDD-", () => {
    const url = "https://www.france24.com/fr/europe-20240319-actualite-ukraine";
    expect(extractDateFromUrl(url)!.toISOString().slice(0, 10)).toBe("2024-03-19");
  });
});
...
describe("/DDMMYY/ (Mediapart)", () => {
  it("extracts date when 2-digit year is in 0-30 range", () => {
    ...
    expect(extractDateFromUrl(url)!.toISOString().slice(0, 10)).toBe("2020-03-15");
  });
