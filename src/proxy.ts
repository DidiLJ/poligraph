 ...
 * Throttled so it fires at most once per window per instance, never per request.
 * In production the signal is a console error (visible in Vercel logs) plus a
 * Sentry warning; outside production a single light log. The Sentry event is
 * flushed via `event.waitUntil` because the isolate can freeze right after the
 * response, which would otherwise drop the in-flight event. No public header is
 * emitted, to avoid telling clients that throttling is off.
 */
 ...
 * response to send (CORS preflight 204 / 429 / 503 / passthrough with headers),
 * or null when the path is not an API route subject to limiting so the proxy
 * continues. Restored here from the former root middleware.ts, which Next 16
 * ignored in this src/ project.
 */
...
// ─── Proxy (Next 16 convention; replaces the ignored root middleware.ts) ──
...
export async function proxy(request: NextRequest, event: NextFetchEvent) {
  ...
  // Per-IP rate limiting for /api routes (restored from the former root middleware.ts).
